...
window.addEventListener("load", () => {
  function button(label, ariaLabel, icon, className) {
    ...
    btn.innerHTML = `<div class="btn-icon__body">${icon}<strong class="btn-icon__label">${label}</strong></div></button>`
    return btn
  }
  ...
  function addLanguage(codeBlockSelector) {
    const codeBlocks = [].slice.call(
      document.querySelectorAll(codeBlockSelector),
    )

    codeBlocks.forEach((block) => {
      const lang = Object.keys(languages).find((className) =>
        block.classList.contains(className),
      )

      if (lang) {
        block.setAttribute("aria-label", languages[lang])
      }
    })
  }
